...
runners.map((person) => {
  firstNamesAllCaps.push(person.first_name.toUpperCase());
});
...
const runnersLargeSizeShirt = runners.filter((person) => {
...
const ticketPriceTotal = runners.reduce((accumuator, person) => {
  return accumuator + person.donation;
}
  , 0);
...
// Problem 1 The director needs a list of runners who have donated more than $200.

let bigmoney = runners.filter((person) => {
  return person.donation > 200;
})

console.log(bigmoney);

// Problem 2 The registration team requested a check-in list with last names in bold.

let lastNamesbold = [];
runners.map((person) => {
  lastNamesbold.push(person.last_name.bold());
});

console.log(lastNamesbold);

// Problem 3 We need to email all the runners and let them know the event is cancelled due to rain.
